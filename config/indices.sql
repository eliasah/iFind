...
-- création des tables
CREATE TABLE t_metadata (meta_id SERIAL PRIMARY KEY, filename VARCHAR(30) NOT NULL, filepath VARCHAR(120) NOT NULL, permission VARCHAR(9) NOT NULL, last_mod DATE NOT NULL);

CREATE TABLE t_index (trg_id VARCHAR(3) NOT NULL,meta_id INT NOT NULL REFERENCES t_metadata(meta_id) ON UPDATE CASCADE, PRIMARY KEY(trg_id, meta_id));
